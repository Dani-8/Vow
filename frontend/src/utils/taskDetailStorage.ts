...
// Helper to resolve seed keys flexibly across exact IDs or task slugs/keywords
function findSeedKey<T>(dict: Record<string, T>, taskIdOrTitle: string): string | undefined {
    if (!taskIdOrTitle) return undefined;
    if (dict[taskIdOrTitle]) return taskIdOrTitle;

    const normalized = taskIdOrTitle.toLowerCase().replace(/[^a-z0-9]/g, '');
    for (const key of Object.keys(dict)) {
        const normKey = key.toLowerCase().replace(/[^a-z0-9]/g, '');
        if (normalized.includes('russian') && normKey.includes('russian')) return key;
        if ((normalized.includes('ui') || normalized.includes('revamp') || normalized.includes('design')) && normKey.includes('ui')) return key;
        if ((normalized.includes('ai') || normalized.includes('llm') || normalized.includes('agent')) && normKey.includes('ai')) return key;
        if ((normalized.includes('mern') || normalized.includes('fullstack')) && normKey.includes('mern')) return key;
    }
    return undefined;
}
...
export function getTaskStickyNotes(taskId: string): TaskStickyNote[] {
    ...
    const seedKey = findSeedKey(DEFAULT_TASK_STICKY_NOTES, taskId);
...
export function getTaskAttachments(taskId: string): TaskAttachment[] {
    ...
    const seedKey = findSeedKey(DEFAULT_TASK_ATTACHMENTS, taskId);
...
export function getTaskActivities(taskId: string): TaskActivityItem[] {
    ...
    const seedKey = findSeedKey(DEFAULT_TASK_ACTIVITIES, taskId);
