...
async function seedDemoData() {
    try {
        ...
        const existingMaps = await TaskMap.find({ userId: demoUser._id });
        if (existingMaps.length === 0) {
            console.log('Seeding demo task maps for demo user...');
            ...
            console.log('Demo task maps seeded successfully.');
        }
    } catch (err) {
